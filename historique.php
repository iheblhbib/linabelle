<?php 
        ...
        $req_his = mysqli_query($con,"SELECT nom_type, nom, num_carte,quantite_achete, prix_achat, date
                                      FROM produits
                                      JOIN type ON (id_type = id_t)
                                      JOIN achat ON (id_p = id_prod)
                                      WHERE id_client=$id_id and id_type = id_t and  id_p = id_prod");
?>
...
<body>
                        ...
                        <table class="table table-striped">
                            ...
                            <tbody>
                                ...
                                while($liste = mysqli_fetch_array($query7)){
                                    ...
                                    <td>'.$price.'</td>                                
                                    <td>'.$quantity.'</td>
                                    <td><a href="historique.php?del='.$idprod.'" class="del"><img src="images/trash.png" ></a></td>
                                    </tr>';
                                } }?>
    ...
    <section class="contact section">
        <div class="container">
            ...
            <div class="col-lg-12">
                    <div class="text-center">
                        <h1 class="text-capitalize mb-0 text-lg">votre historique</h1>
                    </div>
                </div>
                <div class="col-lg-8">
  ...
  <tbody>
      <?php while($hist = mysqli_fetch_array($req_his)){
          $i++;
          echo'
    <tr>
      <th scope="row">'.$i.'</th>
      <td>'.$hist['nom_type'].'</td>
      <td>'.$hist['nom'].'</td>
      <td>'.$hist['num_carte'].'</td>
      <td>'.$hist['quantite_achete'].'</td>
      <td>'.$hist['prix_achat'].'</td>
      <td>'.$hist['date'].'</td>
    ...
    <section class="section-sm pt-0 cta">
        <div class="container">
            <div class="row align-items-center bg-dark p-5">
                <div class="col-lg-8">
                    <h3 class="text-white mb-0">Vous avez un problème ?</h3>
                </div>
